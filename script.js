            ...
            for (let i = 0; i < data.trips.length; i++) {
               ...
               document.querySelector("#result").innerHTML += `
                    <div class="tickets-found">
                        ...
                        <button type="button" id="${data.trips[i]._id}" class="book-btn"><a id="refBook" href="cart.html" >Book</a></button>
                    </div>
                    `;
            }
         }
   ...
   for (let i = 0; i < document.querySelectorAll(".book-btn").length; i++) {
      document
         ...
         [i].addEventListener("click", function () {
            const departure =
               this.parentNode.querySelector(".depart").textContent;
            const arrival =
               this.parentNode.querySelector(".arrival").textContent;
            const date =
               this.parentNode.querySelector(".hidden-date").textContent;
            console.log(date);
            const price = this.parentNode.querySelector(
               ".travel-price >span"
            ).textContent;
